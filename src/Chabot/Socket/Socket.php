...
abstract class Socket {
	...
	{
        if ($domain === AF_UNIX && $protocol === SOL_TCP) {
            $protocol = 0;
        }
		...
		if (!@socket_set_option($this->socket, SOL_SOCKET, SO_REUSEADDR, 1)) {
			throw new Exception("Could not set SO_REUSEADDR: ".$this->get_error());
		}
		if (!@socket_bind($this->socket, $bind_address, $bind_port)) {
			throw new Exception("Could not bind socket to [$bind_address - $bind_port]: ".socket_strerror(socket_last_error($this->socket)));
		}
	...
	public function set_receive_timeout($sec, $usec)
	{
		if (!is_resource($this->socket)) {
			...
			throw new Exception("Could not set socket receive timeout: ".$this->get_error());
		}
	}
